...
Der Framework-Code patcht Il2Cpp-Methoden und ruft `HookBinder.DispatchBefore` / `DispatchAfter` auf. Du registrierst Handler mit dem **exakten** String `greg.PLAYER.ComponentInitialized`:
...
- Ein Hook-**Katalog** / Alias-Datei wurde geladen (z. B. `HookBinder.LoadAliases(pfad)` oder `Hooker.InstallFromCatalog(...)`), sodass die Il2Cpp-Signatur auf den kanonischen String `greg.PLAYER.ComponentInitialized` gemappt ist.
...
Abmelden: `GregEventDispatcher.Off("greg.PLAYER.ComponentInitialized", handler)` (dieselbe Delegaten-Referenz wie bei `On`).
