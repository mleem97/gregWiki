...
The **gregFramework / gregWiki** product naming (site title, navigation, page titles) is separate from this design language. **Luminescent Architect** names this visual design system only.
...
Implementation reference: `gregWiki/src/styles/custom.css` (`@theme` tokens, `.btn-primary`, `.glass-card`, `.navbar`).
...
Semantic colors are defined as Tailwind v4 `@theme` variables in `gregWiki/src/styles/custom.css` (e.g. `--color-primary`, `--color-on-surface`, `--color-surface-container-low`). Prefer those utilities in React pages (`bg-background`, `text-on-surface`, `border-outline-variant/15`) so the site stays aligned with this spec.
